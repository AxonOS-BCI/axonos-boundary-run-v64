...
[![Launch Game](https://img.shields.io/badge/▶_Launch_Game-AxonOS_Boundary_Run_v64-c8ad6b?style=for-the-badge)](https://axonos-bci.github.io/axonos-boundary-run-v64/)
[![Release](https://img.shields.io/badge/Release-v64.0.0-111827?style=for-the-badge)](https://github.com/AxonOS-BCI/axonos-boundary-run-v64/releases/tag/v64.0.0)
[![Creator](https://img.shields.io/badge/Creator-Denis_Yermakou-0b1220?style=for-the-badge)](ATTRIBUTION.md)
...
Boundary Run v64 uses a **botless contributor** publishing model. The site is deployed from the `gh-pages` branch by a local maintainer command, not by `github-actions[bot]`.

```bash
bash scripts/deploy_pages_branch.sh
```
...
CI must not create commits. GitHub Actions run read-only verification only. Pages publishing is performed from the maintainer machine so contributor graphs remain attributable to human maintainers. Dependabot may open PRs, but dependency changes should be re-committed manually by Denis Yermakou before merge if a botless contributor graph is required.
