...
need_cmd node; need_cmd python3; need_cmd zip; need_cmd tar; need_cmd sha256sum
rm -rf dist "$ASSETS" .release-tmp
mkdir -p "$ASSETS"
...
mkdir -p "$TMP"
tar --exclude='.git' --exclude='.release-tmp' --exclude='release-assets' --exclude='dist' --exclude='dist_build1' --exclude='dist_build2' --exclude='.DS_Store' -cf - . | tar -xf - -C "$TMP"
(
  cd .release-tmp
